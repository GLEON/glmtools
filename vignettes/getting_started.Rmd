...
```{r}
eg_nml <- read_nml(nml_file = file.path(GLM3r_folder,'glm3.nml'))
```
...
```{r, fig.width=4, fig.height=2.5, results = 'hide', fig.keep='first'}
nc_file <- file.path(GLM3r_folder, 'output/output.nc')
plot_var(nc_file = nc_file, var_name = 'temp')
```

### plotting multiple variables
```{r, fig.width=4, fig.height=5, results = 'hide', fig.keep='first'}
...
field_file <- file.path(glmtools_folder, 'LakeMendota_field_data.csv')
```
...
plot_var_compare(nc_file, field_file, var_name = 'temp') ## makes a plot!
```

### There are many ways to customize these graphs
```{r, fig.width=6, fig.height=5.25, eval = T, results = 'hide', fig.keep='first'}
plot_var_compare(nc_file, field_file, var_name = 'temp',interval = 2, 
